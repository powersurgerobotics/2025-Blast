...
async def runMission():
    yaw_value = 0
    speed_percent = 90
    await Gyro_Move_Forward_By_Distance(yaw_value,6,speed_percent) # Move Straight until the specified distance point and stop.
    await Gyro_Right_turn(18)
    await Gyro_Move_Forward_By_Distance(yaw_value,28, speed_percent)
    await Move_Right_Attachment_Forward(40,18)
    
    await Gyro_Move_Backward_By_Distance(yaw_value,5, speed_percent)
