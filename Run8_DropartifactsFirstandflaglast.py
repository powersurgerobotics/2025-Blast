...
async def runMission():
    yaw_value = 0
    speed_percent = 90
    await Gyro_Move_Forward_By_Distance(yaw_value,38,speed_percent) # Move Straight until the specified distance point and stop.
    await Move_Left_Attachment_Backward(40,25)
    speed_percent = 70
    await Gyro_Move_Backward_By_Distance(yaw_value, 3, speed_percent)
    await Gyro_Right_turn(85.5)
    speed_percent = 75
    
    await Gyro_Move_Forward_By_Distance(yaw_value,15.75, speed_percent)
    await Move_Right_Attachment_Forward(35,28)
